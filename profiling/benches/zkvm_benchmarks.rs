...
mod risc0_benchmarks {
    ...
    pub fn benchmark_risc0_email_verification() {
        let test_email = zkemail_profiling::create_test_email(10); // 10KB test email

        println!("RISC0 Email Verification Benchmark");
        println!("Email size: {}KB", test_email.raw_email.len() / 1024);

        let metrics = performance::measure_email_verification_performance(&test_email);
...
mod sp1_benchmarks {
    ...
    pub fn benchmark_sp1_email_verification() {
        ...
        // Attempt to execute in SP1 zkVM for actual measurements
        match execute_sp1_program(test_email.clone()) {
            Ok(result) => {
                println!("SP1 execution successful!");
                println!("Actual zkVM cycles: {}", result.cycle_count);
                println!(
                    "Email verification: {}",
            ...
            Err(e) => {
                println!(
                    "SP1 execution failed (this is expected without proper setup): {}",
                    e
                );
                println!("To enable full SP1 integration:");
                println!("1. Build the SP1 program: cd profiling/sp1 && ./build.sh");
                println!("2. Run with: cargo run --features sp1 --bin sp1-script");
            }
        }
    }
}
