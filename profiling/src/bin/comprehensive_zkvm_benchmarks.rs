...
use zkemail_profiling::zkvm_integration::performance::measure_email_verification_performance;
...
fn benchmark_email_size(size_kb: usize, iterations: usize) -> BenchmarkResult {
    println!(
        "Benchmarking email size: {}KB (iterations: {})",
        size_kb, iterations
    );
...
fn run_comprehensive_benchmarks() -> Vec<BenchmarkResult> {
    ...
    for &size in &email_sizes {
        ...
        println!(
            "Size: {}KB | Time: {}ms | Cycles: {} | Memory: {}KB | Throughput: {:.2} emails/sec",
...
fn analyze_performance_trends(results: &[BenchmarkResult]) {
    ...
    if results.len() >= 2 {
        ...
        println!(
            "Scaling Analysis ({}KB -> {}KB):",
            first.email_size_kb, last.email_size_kb
        );
        ...
        println!(
            "  Time/Size efficiency: {:.2}x (lower is better)",
            time_ratio / size_ratio
        );
        println!(
            "  Cycles/Size efficiency: {:.2}x (lower is better)",
            cycles_ratio / size_ratio
        );
        println!(
            "  Memory/Size efficiency: {:.2}x (lower is better)",
    ...
    println!(
        "  Highest throughput: {:.2} emails/sec at {}KB",
        best_throughput, best_size
    );
    println!(
        "  Best time efficiency: {:.2} ms/KB at {}KB",
        best_efficiency, best_efficiency_size
    );
    println!();
}
...
fn save_benchmark_results(results: &[BenchmarkResult]) -> Result<(), std::io::Error> {
    ...
    if results.len() >= 2 {
        ...
        writeln!(
            file,
            "- Cycles/Size efficiency: {:.2}x",
            cycles_ratio / size_ratio
        )?;
        writeln!(
            file,
            "- Memory/Size efficiency: {:.2}x",
            memory_ratio / size_ratio
        )?;
    }
